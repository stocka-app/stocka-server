...
describe('PATCH /api/storages/warehouses/:uuid/convert-to-custom-room (E2E — H-07)', () => {
  let app: INestApplication;
  let dataSource: DataSource;
  const TENANT_NAME = 'CvtWhToCr E2E';
  const OWNER_EMAIL = 'cvt.wh.to.cr.owner@example.com';
  const OWNER_USERNAME = 'cvtwhtocrowner';
  ...
  describe('Given an active warehouse', () => {
    ...
    beforeAll(async () => {
      uuid = await createWarehouse(app, ownerToken, 'CvtWhToCr Alpha');
    });

    it('Then convert-to-custom-room returns 200 and subsequent GET shows CUSTOM_ROOM', async () => {
      const res = await request(app.getHttpServer())
  ...
  describe('Given an archived warehouse', () => {
    ...
    beforeAll(async () => {
      uuid = await createWarehouse(app, ownerToken, 'CvtWhToCr Archived');
      await request(app.getHttpServer())
  ...
  describe('Given a frozen warehouse', () => {
    ...
    beforeAll(async () => {
      uuid = await createWarehouse(app, ownerToken, 'CvtWhToCr Frozen');
      await request(app.getHttpServer())
  ...
  describe('Given an unauthenticated client', () => {
    it('Then convert returns 401', async () => {
      ...
      const res = await request(app.getHttpServer()).patch(
        `/api/storages/warehouses/${fakeUUID}/convert-to-custom-room`,
      );
  ...
  describe('Given a non-existent warehouse UUID', () => {
    it('Then convert returns 404', async () => {
      const fakeUUID = '00000000-0000-0000-0000-000000000000';
      const res = await request(app.getHttpServer())
        .patch(`/api/storages/warehouses/${fakeUUID}/convert-to-custom-room`)
        .set('Authorization', `Bearer ${ownerToken}`);
