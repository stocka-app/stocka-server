...
describe('POST /api/storages/store-rooms/:uuid/freeze (E2E)', () => {
  let app: INestApplication;
  let dataSource: DataSource;

  const TENANT_NAME = 'FreezeStoreRoom E2E Business';
  const OWNER_EMAIL = 'freeze.storeroom.owner@example.com';
  const OWNER_USERNAME = 'freezestoreroomowner';
  ...
  beforeAll(async () => {
    ...
    storeRoomUUID = await createStoreRoom(app, ownerToken, 'FreezeStoreRoom Beta');
  });
  ...
  describe('Given an owner with an active store room', () => {
    describe('When POST /api/storages/store-rooms/:uuid/freeze is called', () => {
      it('Then it returns 200 and the store room transitions to FROZEN status', async () => {
        const res = await request(app.getHttpServer())
          .post(`/api/storages/store-rooms/${storeRoomUUID}/freeze`)
  ...
  describe('Given a store room that is already frozen', () => {
    describe('When POST /api/storages/store-rooms/:uuid/freeze is called again', () => {
  ...
  describe('Given an unauthenticated client', () => {
    describe('When POST /api/storages/store-rooms/:uuid/freeze is called without a token', () => {
      it('Then it returns 401', async () => {
        const res = await request(app.getHttpServer()).post(
          `/api/storages/store-rooms/${storeRoomUUID}/freeze`,
        );
