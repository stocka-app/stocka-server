...
async function createCustomRoom(
  ...
  const res = await request(app.getHttpServer())
    ...
    .set('Authorization', `Bearer ${token}`)
    .send({ name, roomType: 'Office', icon: 'apartment', color: '#3B82F6' });
...
async function freezeCustomRoom(
  app: INestApplication,
  token: string,
  uuid: string,
): Promise<void> {
...
describe('DELETE /api/storages/custom-rooms/:uuid/permanent (E2E — H-08)', () => {
  let app: INestApplication;
  let dataSource: DataSource;

  const TENANT_NAME = 'DeletePermCustomRoom E2E Business';
  const OWNER_EMAIL = 'delete.perm.customroom.owner@example.com';
  const OWNER_USERNAME = 'deletepermcustomroomowner';

  const OTHER_TENANT_NAME = 'Other CustomRoom Tenant';
  const OTHER_OWNER_EMAIL = 'other.customroom.owner@example.com';
  const OTHER_OWNER_USERNAME = 'othercustomroomowner';
  ...
  describe('Given an ARCHIVED custom-room', () => {
    ...
    beforeAll(async () => {
      uuid = await createCustomRoom(app, ownerToken, 'Perm-Delete Alpha CR');
      await archiveCustomRoom(app, ownerToken, uuid);
    });

    describe('When permanent delete is requested', () => {
      it('Then it returns 204 NO_CONTENT and the custom-room no longer exists', async () => {
        const res = await request(app.getHttpServer())
          .delete(`/api/storages/custom-rooms/${uuid}/permanent`)
  ...
  describe('Given an ACTIVE custom-room', () => {
    ...
    beforeAll(async () => {
      uuid = await createCustomRoom(app, ownerToken, 'Perm-Delete Active CR');
    });

    describe('When permanent delete is requested', () => {
      it('Then it returns 409 STORAGE_NOT_ARCHIVED and the custom-room remains untouched', async () => {
        const res = await request(app.getHttpServer())
          .delete(`/api/storages/custom-rooms/${uuid}/permanent`)
          .set('Authorization', `Bearer ${ownerToken}`);
  ...
  describe('Given a FROZEN custom-room', () => {
    ...
    beforeAll(async () => {
      uuid = await createCustomRoom(app, ownerToken, 'Perm-Delete Frozen CR');
      await freezeCustomRoom(app, ownerToken, uuid);
    });
  ...
  describe('Given a non-existent custom-room UUID', () => {
    describe('When permanent delete is requested', () => {
      it('Then it returns 404 STORAGE_NOT_FOUND', async () => {
        ...
        const res = await request(app.getHttpServer())
          .delete(`/api/storages/custom-rooms/${fakeUUID}/permanent`)
          .set('Authorization', `Bearer ${ownerToken}`);
  ...
  describe('Given a custom-room owned by another tenant', () => {
    ...
    beforeAll(async () => {
      otherTenantUUID = await createCustomRoom(app, otherOwnerToken, 'Cross-Tenant CR');
      await archiveCustomRoom(app, otherOwnerToken, otherTenantUUID);
    });
    ...
    describe('When permanent delete is attempted from a different tenant', () => {
      it('Then it returns 404 STORAGE_NOT_FOUND (tenant isolation)', async () => {
        const res = await request(app.getHttpServer())
          .delete(`/api/storages/custom-rooms/${otherTenantUUID}/permanent`)
          .set('Authorization', `Bearer ${ownerToken}`);
    ...
    describe('When permanent delete is called without a token', () => {
      it('Then it returns 401 UNAUTHORIZED', async () => {
        ...
        const res = await request(app.getHttpServer()).delete(
          `/api/storages/custom-rooms/${fakeUUID}/permanent`,
        );
  ...
  describe('Given the same UUID is permanently deleted twice in rapid succession (concurrency)', () => {
    ...
    beforeAll(async () => {
      uuid = await createCustomRoom(app, ownerToken, 'Perm-Delete Concurrency CR');
      await archiveCustomRoom(app, ownerToken, uuid);
