...
describe('PATCH /api/storages/store-rooms/:uuid/convert-to-custom-room (E2E — H-07)', () => {
  let app: INestApplication;
  let dataSource: DataSource;
  const TENANT_NAME = 'CvtSrToCr E2E';
  const OWNER_EMAIL = 'cvt.sr.to.cr.owner@example.com';
  const OWNER_USERNAME = 'cvtsrtocrowner';
  ...
  describe('Given an active store room', () => {
    ...
    beforeAll(async () => {
      uuid = await createStoreRoom(app, ownerToken, 'CvtSrToCr Alpha');
    });

    it('Then convert-to-custom-room returns 200 and GET shows CUSTOM_ROOM', async () => {
      const res = await request(app.getHttpServer())
  ...
  describe('Given an archived store room', () => {
    ...
    beforeAll(async () => {
      uuid = await createStoreRoom(app, ownerToken, 'CvtSrToCr Archived');
      await request(app.getHttpServer())
  ...
  describe('Given a frozen store room', () => {
    ...
    beforeAll(async () => {
      uuid = await createStoreRoom(app, ownerToken, 'CvtSrToCr Frozen');
      await request(app.getHttpServer())
  ...
  describe('Given an unauthenticated client', () => {
    it('Then convert returns 401', async () => {
      ...
      const res = await request(app.getHttpServer()).patch(
        `/api/storages/store-rooms/${fakeUUID}/convert-to-custom-room`,
      );
  ...
  describe('Given a non-existent store room UUID', () => {
    it('Then convert returns 404', async () => {
      const fakeUUID = '00000000-0000-0000-0000-000000000000';
      const res = await request(app.getHttpServer())
        .patch(`/api/storages/store-rooms/${fakeUUID}/convert-to-custom-room`)
        .set('Authorization', `Bearer ${ownerToken}`);
