...
describe('PATCH /api/storages/custom-rooms/:uuid/convert-to-warehouse (E2E — H-07)', () => {
  let app: INestApplication;
  let dataSource: DataSource;
  const TENANT_NAME = 'CvtCrToWh E2E';
  const OWNER_EMAIL = 'cvt.cr.to.wh.owner@example.com';
  const OWNER_USERNAME = 'cvtcrtowhowner';
  ...
  describe('Given an active custom room with a valid address', () => {
    ...
    beforeAll(async () => {
      uuid = await createCustomRoom(app, ownerToken, 'CvtCrToWh Alpha');
    });

    it('Then convert-to-warehouse returns 200 and GET shows WAREHOUSE', async () => {
      const res = await request(app.getHttpServer())
  ...
  describe('Given an archived custom room', () => {
    ...
    beforeAll(async () => {
      uuid = await createCustomRoom(app, ownerToken, 'CvtCrToWh Archived');
      await request(app.getHttpServer())
  ...
  describe('Given a frozen custom room', () => {
    ...
    beforeAll(async () => {
      uuid = await createCustomRoom(app, ownerToken, 'CvtCrToWh Frozen');
      await request(app.getHttpServer())
  ...
  describe('Given an unauthenticated client', () => {
    it('Then convert returns 401', async () => {
      ...
      const res = await request(app.getHttpServer()).patch(
        `/api/storages/custom-rooms/${fakeUUID}/convert-to-warehouse`,
      );
  ...
  describe('Given a non-existent custom room UUID', () => {
    it('Then convert returns 404', async () => {
      const fakeUUID = '00000000-0000-0000-0000-000000000000';
      const res = await request(app.getHttpServer())
        .patch(`/api/storages/custom-rooms/${fakeUUID}/convert-to-warehouse`)
        .set('Authorization', `Bearer ${ownerToken}`);
