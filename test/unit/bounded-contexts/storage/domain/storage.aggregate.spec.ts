...
// ── freezeWarehouse / freezeStoreRoom / freezeCustomRoom ──────────────────────

describe('StorageAggregate — freeze items', () => {
  describe('Given an aggregate with an active warehouse', () => {
    let aggregate: StorageAggregate;
    ...
    describe('When freezeWarehouse is called with a matching UUID', () => {
      it('Then the model is frozen and a StorageFrozenEvent is applied', () => {
        aggregate.freezeWarehouse(WH_UUID, ACTOR_UUID);

        const wh = aggregate.findWarehouse(WH_UUID)!;
        expect(wh.isFrozen()).toBe(true);

        const events = aggregate.getUncommittedEvents();
        expect(events).toHaveLength(1);
        expect(events[0]).toBeInstanceOf(StorageFrozenEvent);
        const event = events[0] as StorageFrozenEvent;
        expect(event.storageUUID).toBe(WH_UUID);
        expect(event.tenantUUID).toBe(TENANT_UUID);
        expect(event.actorUUID).toBe(ACTOR_UUID);
      });
    ...
    describe('When freezeWarehouse is called with a non-matching UUID', () => {
      it('Then nothing happens (no-op)', () => {
        aggregate.freezeWarehouse('019538a0-0000-7000-8000-999999999999', ACTOR_UUID);

        const wh = aggregate.findWarehouse(WH_UUID)!;
        expect(wh.isFrozen()).toBe(false);
        expect(aggregate.getUncommittedEvents()).toHaveLength(0);
      });
    });
  });

  describe('Given an aggregate with an active store room', () => {
    let aggregate: StorageAggregate;
    ...
    describe('When freezeStoreRoom is called with a matching UUID', () => {
      it('Then the model is frozen and a StorageFrozenEvent is applied', () => {
        aggregate.freezeStoreRoom(SR_UUID, ACTOR_UUID);

        const sr = aggregate.findStoreRoom(SR_UUID)!;
        expect(sr.isFrozen()).toBe(true);
    ...
    describe('When freezeStoreRoom is called with a non-matching UUID', () => {
      it('Then nothing happens (no-op)', () => {
        aggregate.freezeStoreRoom('019538a0-0000-7000-8000-999999999999', ACTOR_UUID);

        const sr = aggregate.findStoreRoom(SR_UUID)!;
        expect(sr.isFrozen()).toBe(false);
        expect(aggregate.getUncommittedEvents()).toHaveLength(0);
      });
    });
  });

  describe('Given an aggregate with an active custom room', () => {
    let aggregate: StorageAggregate;
    ...
    describe('When freezeCustomRoom is called with a matching UUID', () => {
      it('Then the model is frozen and a StorageFrozenEvent is applied', () => {
        aggregate.freezeCustomRoom(CR_UUID, ACTOR_UUID);

        const cr = aggregate.findCustomRoom(CR_UUID)!;
        expect(cr.isFrozen()).toBe(true);
    ...
    describe('When freezeCustomRoom is called with a non-matching UUID', () => {
      it('Then nothing happens (no-op)', () => {
        aggregate.freezeCustomRoom('019538a0-0000-7000-8000-999999999999', ACTOR_UUID);

        const cr = aggregate.findCustomRoom(CR_UUID)!;
        expect(cr.isFrozen()).toBe(false);
        expect(aggregate.getUncommittedEvents()).toHaveLength(0);
      });
    });
  });
...
// ── unfreezeWarehouse / unfreezeStoreRoom / unfreezeCustomRoom ─────────────────

describe('StorageAggregate — unfreeze items', () => {
  describe('Given an aggregate with a frozen warehouse', () => {
    let aggregate: StorageAggregate;

    beforeEach(() => {
      aggregate = StorageAggregate.reconstitute({
        id: 42,
        uuid: '019538a0-0000-7000-8000-000000000099',
        tenantUUID: TENANT_UUID,
    ...
    describe('When unfreezeWarehouse is called with a matching UUID', () => {
      it('Then the model is unfrozen and a StorageReactivatedEvent is applied', () => {
        aggregate.unfreezeWarehouse(WH_UUID, ACTOR_UUID);
        ...
        const events = aggregate.getUncommittedEvents();
        expect(events).toHaveLength(1);
        expect(events[0]).toBeInstanceOf(StorageReactivatedEvent);
        const event = events[0] as StorageReactivatedEvent;
        expect(event.storageUUID).toBe(WH_UUID);
        expect(event.tenantUUID).toBe(TENANT_UUID);
        expect(event.actorUUID).toBe(ACTOR_UUID);
      });
    ...
    describe('When unfreezeWarehouse is called with a non-matching UUID', () => {
      it('Then nothing happens (no-op)', () => {
        aggregate.unfreezeWarehouse('019538a0-0000-7000-8000-999999999999', ACTOR_UUID);
  ...
  describe('Given an aggregate with a frozen store room', () => {
    let aggregate: StorageAggregate;
    ...
    describe('When unfreezeStoreRoom is called with a matching UUID', () => {
      it('Then the model is unfrozen and a StorageReactivatedEvent is applied', () => {
        aggregate.unfreezeStoreRoom(SR_UUID, ACTOR_UUID);
    ...
    describe('When unfreezeStoreRoom is called with a non-matching UUID', () => {
      it('Then nothing happens (no-op)', () => {
        aggregate.unfreezeStoreRoom('019538a0-0000-7000-8000-999999999999', ACTOR_UUID);
  ...
  describe('Given an aggregate with a frozen custom room', () => {
    let aggregate: StorageAggregate;
    ...
    describe('When unfreezeCustomRoom is called with a matching UUID', () => {
      it('Then the model is unfrozen and a StorageReactivatedEvent is applied', () => {
        aggregate.unfreezeCustomRoom(CR_UUID, ACTOR_UUID);
    ...
    describe('When unfreezeCustomRoom is called with a non-matching UUID', () => {
      it('Then nothing happens (no-op)', () => {
        aggregate.unfreezeCustomRoom('019538a0-0000-7000-8000-999999999999', ACTOR_UUID);
