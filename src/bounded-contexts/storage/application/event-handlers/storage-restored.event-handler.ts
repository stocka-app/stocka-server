...
@Injectable()
@EventsHandler(StorageRestoredEvent)
export class StorageRestoredEventHandler implements IEventHandler<StorageRestoredEvent> {
  constructor(
  ...
  async handle(event: StorageRestoredEvent): Promise<void> {
    const entry = StorageActivityLogEntry.create({
      ...
      action: StorageActivityAction.RESTORED,
      previousValue: null,
      newValue: null,
    });
