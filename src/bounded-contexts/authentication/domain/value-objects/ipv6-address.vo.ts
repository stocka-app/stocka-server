...
import { IPV6_HEX_GROUP_PATTERN } from '@common/constants/validation.constants';
...
export class IPv6AddressVO extends StringVO {
  ...
  private static validateIPv6(value: string): boolean {
    ...
    return groups.length === 8 && groups.every((g) => IPV6_HEX_GROUP_PATTERN.test(g));
  }
  ...
  private static validateCompressedIPv6(left: string, right: string): boolean {
    ...
    return (
      leftGroups.every((g) => IPV6_HEX_GROUP_PATTERN.test(g)) &&
      rightGroups.every((g) => IPV6_HEX_GROUP_PATTERN.test(g))
    );
  }
