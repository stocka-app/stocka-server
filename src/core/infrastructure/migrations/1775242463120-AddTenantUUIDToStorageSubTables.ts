...
export class AddTenantUUIDToStorageSubTables1775242463120 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "storage"."storage_activity_log" ADD "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()`,
    );
    await queryRunner.query(
      `ALTER TABLE "storage"."storage_activity_log" ADD "updated_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()`,
    );
    await queryRunner.query(
      `ALTER TABLE "storage"."storage_activity_log" ADD "archived_at" TIMESTAMP WITH TIME ZONE`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "storage"."storage_activity_log" DROP COLUMN "archived_at"`,
    );
    await queryRunner.query(
      `ALTER TABLE "storage"."storage_activity_log" DROP COLUMN "updated_at"`,
    );
    await queryRunner.query(
      `ALTER TABLE "storage"."storage_activity_log" DROP COLUMN "created_at"`,
    );
